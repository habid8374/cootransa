    ...
    <motion.div ref={ref as any} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="relative group">
      <div className="relative p-8 rounded-2xl bg-white/5 border border-white/10 hover:border-green-500/30 transition-all duration-300 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="text-4xl sm:text-5xl lg:text-6xl font-black font-display bg-gradient-to-r from-green-400 to-green-300 bg-clip-text text-transparent mb-2">{isText ? textValue : `${count}${suffix}`}</div>
    ...
    <section className="py-24 bg-[#0A0F1E] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black font-display text-white mt-3">Más de cinco décadas<br/><span className="bg-gradient-to-r from-green-400 to-purple-400 bg-clip-text text-transparent">de movilidad continua</span></h2>
        </motion.div>
