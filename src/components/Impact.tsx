    ...
    <>
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <span className="text-yellow-600 text-sm font-semibold tracking-widest uppercase">Impacto social</span>
            <h2 className="text-4xl lg:text-5xl font-black font-display text-gray-900 mt-3">Transformamos vidas,<br/><span className="bg-gradient-to-r from-yellow-500 to-green-500 bg-clip-text text-transparent">construimos región.</span></h2>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {impacts.map((item, i) => (
              <motion.div key={item.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i*0.1 }} className="group text-center p-8 rounded-2xl bg-gray-50 border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all duration-300 hover:-translate-y-1">
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center mx-auto mb-5 shadow-md`}><item.icon size={24} className="text-white"/></div>
      ...
      <section className="py-20 bg-gray-50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center text-gray-400 text-sm font-semibold tracking-widest uppercase">Nuestros aliados estratégicos</motion.p>
        </div>
            ...
            {allies.map((ally, i) => (
              ...
              >
                <motion.div
                  ...
                  className="relative h-32 sm:h-36 rounded-2xl bg-white border border-gray-200 shadow-md flex items-center justify-center p-6 overflow-hidden hover:shadow-lg hover:border-green-200 transition-all duration-300"
                >
                  ...
                  />
                  <span className="hidden absolute inset-0 items-center justify-center text-center px-3 text-gray-700 font-bold text-sm">{ally.name}</span>
                </motion.div>
                <p className="mt-3 text-center text-gray-500 text-xs sm:text-sm font-medium">{ally.name}</p>
              </motion.div>
            ))}
          </div>
        </div>
