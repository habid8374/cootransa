    ...
    <section className="py-24 bg-[#070C1A] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <span className="text-green-400 text-sm font-semibold tracking-widest uppercase">Lo que ofrecemos</span>
          <h2 className="text-4xl lg:text-5xl font-black font-display text-white mt-3">Servicios de transporte<br/><span className="bg-gradient-to-r from-green-400 to-purple-400 bg-clip-text text-transparent">para cada necesidad</span></h2>
          <p className="text-zinc-400 mt-4 max-w-2xl mx-auto text-lg">Desde el primer día de clases hasta el viaje de negocios más importante, COOTRANSA está presente.</p>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, i) => (
            <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i*0.08 }} whileHover={{ scale: 1.03 }} className="group relative p-7 rounded-2xl bg-white/5 border border-white/10 hover:border-green-500/40 transition-all duration-300 cursor-default overflow-hidden">
              ...
              <div className={`relative z-10 w-12 h-12 rounded-xl bg-gradient-to-br ${service.gradient} flex items-center justify-center mb-5 shadow-lg`}><service.icon size={22} className="text-white"/></div>
              <h3 className="relative z-10 text-white font-bold text-xl mb-3 font-display">{service.title}</h3>
              <p className="relative z-10 text-zinc-400 text-sm leading-relaxed">{service.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
